...
4. **The user reviews and merges — don't merge a PR unless asked.** CI must be green; OpenAI Codex auto-reviews each new PR (P0/P1 issues only, steered by the "Review guidelines" section of `AGENTS.md`) — handle its feedback with the `codex-review` skill (`copilot-review` covers GitHub Copilot feedback, if any; `review-sweep` fans out worktree subagents to handle several PRs' feedback at once). For deeper review the user may run `/code-review ultra <PR#>`. On merge the branch auto-deletes and `Fixes #<n>` closes the issue, if there is one.
...
- **Branch names must be ASCII.** Issue titles here often contain IPA characters, and `gh issue develop` copies the title into the branch name — always pass an explicit name: `gh issue develop <n> --name <ascii-name> --checkout`. Don't plan on renaming later: pushing the branch under a new name and deleting the old one closes any open PR from it, which then has to be recreated (as happened with PR #91, recreated as #94).
