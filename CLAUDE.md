...
4. **The user reviews and merges — don't merge a PR unless asked.** CI must be green; OpenAI Codex auto-reviews each new PR (P0/P1 issues only, steered by the "Review guidelines" section of `AGENTS.md`) — handle bot feedback (Codex and Copilot alike) with the `review-feedback` skill; `review-sweep` fans out worktree subagents to handle several PRs' feedback at once. For deeper review the user may run `/code-review ultra <PR#>`. On merge the branch auto-deletes and `Fixes #<n>` closes the issue, if there is one.
