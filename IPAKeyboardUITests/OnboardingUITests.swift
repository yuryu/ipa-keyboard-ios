...
//  first-run auto-presentation (including the Full-Access-not-required
//  messaging), and the force-skip path plus its persistent reopen
//  affordance.  Each XCUITest costs a full cold app launch in CI, so
//  scenarios sharing a launch configuration are exercised in one test
//  rather than one test per assertion (issue #43).
//
...
final class OnboardingUITests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        // IPAKeyboardUITests: a developer's simulator may be left in
        // landscape, where lazy lists drop off-screen rows from the
        // accessibility tree.
    ...
    // MARK: - First-run auto-presentation, guidance content + dismissal

    /// With `--uitest-show-onboarding`, the guidance sheet auto-presents on
    /// launch — regardless of whatever first-run state the simulator's
    /// UserDefaults already holds — states plainly that Full Access is NOT
    /// required for typing, offers the Settings deep link, and Done
    /// dismisses it back to the layout library.  One launch covers all of
    /// these because they share the force-show configuration.
    @MainActor
    func test_onboarding_forceShow_showsGuidance_andCanBeDismissed() throws {
    ...
    // MARK: - Force-skip path + persistent affordance

    /// With `--uitest-skip-onboarding`, the layout library appears
    /// immediately and the onboarding sheet never auto-presents; the
    /// toolbar help button then reopens the same guidance on demand.
    /// One launch covers both because they share the force-skip
    /// configuration.
    @MainActor
    func test_onboarding_forceSkip_showsList_andHelpButtonReopensGuidance() throws {
