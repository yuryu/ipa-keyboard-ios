...
- **Degrade gracefully without the App Group.** The store may fall back to bundled defaults with a nil container (the unhosted kit-test runner, or any `CODE_SIGNING_ALLOWED=NO` build — CI's app lanes sign ad-hoc and do have one). The UI must still load and present built-ins; saving may be unavailable in that state — handle it without crashing and ideally tell the user why.
