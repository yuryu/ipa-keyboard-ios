...
- **`KeyboardExtension/` files compile into the kit, not the appex.** The file-system-synchronized folder belongs to the **IPAKeyboardKit** target; exception sets in `project.pbxproj` carve out only `KeyboardViewController.swift` (extension target) and `Info.plist`. So a new file dropped into `KeyboardExtension/` becomes kit code — it must satisfy `APPLICATION_EXTENSION_API_ONLY` and `.swiftinterface` verification, and its internal types are invisible to the extension module. Code that must live in the extension target goes inside `KeyboardViewController.swift`, or the orchestrator/user adds a Target Membership exception in Xcode — you must not edit `project.pbxproj`.
- **The alternates popup's gestures are UIKit by necessity** (`AlternatesPressTracker` overlay in KeyboardView.swift, same pattern as the globe key's `nextKeyboardOverlay`). Don't "simplify" back to SwiftUI gestures — verified empirically on the iOS 26 SDK: `.onLongPressGesture` delivers no callback at physical finger-up after completing; `LongPressGesture.sequenced(before: DragGesture)` **freezes List/ScrollView scrolling** over the keys (even via `.simultaneousGesture`, even with nonzero `minimumDistance`); SwiftUI taps fail on movement, not duration, so a stationary hold still fires a tap on release — UIKit `tap.require(toFail: longPress)` prevents double-typing by construction. Any key interaction needing press-phase locations plus scroll cooperation belongs in a UIKit recognizer overlay.
- **Nothing a recognizer commit path reads may depend on a SwiftUI render pass landing between recognizer callbacks** (e.g. `onGeometryChange` frames for a view mounted at `.began`): on a starved CI runner the queued finger-up can process before the render commits, and the commit silently no-ops against null frames. Pre-mount such overlays invisibly at touch-down (opacity 0 + `accessibilityHidden`) so measurement happens in the touch-down turn's commit; revealing later is a pure opacity flip. These starvation failures do NOT reproduce locally (slowing animations doesn't starve the main thread) — diagnose via the CI xcodebuild activity log (multi-second gaps inside "Synthesize event" mean the app couldn't idle mid-gesture) and temporary NSLog instrumentation read back with `simctl spawn <sim> log show --predicate ...`.
- **Apply per-element accessibility modifiers *before* `.overlay(...)`** — accessibility modifiers applied after an overlay stamp their identifier/label onto every element inside it.
- **VoiceOver custom actions can't be test-automated:** XCUITest has no API to enumerate or invoke them, and Accessibility Inspector can't be driven from this environment. Extract the name/action mapping into a pure kit helper (the existing pattern: `AlternatesAccessibility`), unit-test that, and state plainly in your report that on-device VoiceOver verification remains manual.
- **Simulator visual verification** (the extension can't run unsigned, so host-app previews on a dedicated simulator are the visual-evidence path): `xcodebuild test` **clones** the simulator unless you pass `-parallel-testing-enabled NO`; `simctl` cannot rotate a device and the captured framebuffer stays portrait-dimensioned — landscape evidence needs a throwaway XCUITest that sets `XCUIDevice.shared.orientation` and attaches a screenshot exported via `xcrun xcresulttool export attachments`; `simctl ui <udid> appearance dark` + host-app screenshots covers both color schemes; launch with `--uitest-skip-onboarding` or the sheet blocks first-run screenshots. Mid-gesture visuals (the popup exists only while a finger is down, and XCUITest gesture calls block): run the UI test in the background and take a `simctl io screenshot` burst (~3 fps) from the host, then locate the interesting frames by PNG file-size clustering.
