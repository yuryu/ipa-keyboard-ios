...
- Byte-verify authored JSON with ripgrep `\x{XXXX}` patterns (positive presence + negative `"text": "[g:?'!|]"` checks). Strongest closing check: a negated whitelist class of the shape `[^\x{0000}-\x{007F}\x{0261}\x{02D0}<one \x{XXXX} per remaining intended scalar>]` — build the full list from the file under review each time (it's per-layout, so no reusable literal exists); zero matches then proves the file contains *only* accounted-for code points (used to clear en-GB, 2026-07-02). Related: [[layout-authoring-decisions]].
