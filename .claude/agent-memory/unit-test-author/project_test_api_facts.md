...
The exact Swift Testing summary line on this toolchain (verified 2026-07-12, iOS 26.5 sim) is `✔ Test run with 435 tests in 42 suites passed after 0.598 seconds.` — the N *includes* skipped tests (430 passed + 5 skipped = 435). Skips print as `➜ Test name() skipped: "comment"`. CI's zero-tests guard (issue #188) greps `Test run with [1-9][0-9]* test` after each unit lane; keep any new lane consistent.

`AppGroup.containerURL` is **non-nil on the simulator even for unsigned test runners** (`CODE_SIGNING_ALLOWED=NO` — the simulator doesn't enforce App Group entitlements and creates the container on demand; verified 2026-07-12 on a fresh device). So `@Test(.enabled(if: AppGroup.containerURL == nil, ...))` tests (the "ContainerUnavailable" family in LayoutStoreTests/LayoutTransferTests) skip in every local run, signed or not — their real coverage lives in the `LayoutStore(containerURL: nil)` injection twins. Don't be surprised by 5 skips in a green kit run, and never assert on the real container's contents.
