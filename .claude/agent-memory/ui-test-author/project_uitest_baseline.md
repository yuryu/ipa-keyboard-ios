...
Baseline UITests updated 2026-08-02. build-for-testing passes on iPhone 17 (OS 26.5).
...
`LayoutDetailView`'s `List` (metadata + live `KeyboardView` preview + "Use this Layout" + "Customize symbols", all *ahead* of the action section) and `LayoutListView`'s `List` ("Active" + "Built-in" sections ahead of "My Layouts") can both be taller than one screen — confirmed via the runtime accessibility snapshot (a "2 pages" vertical scroll bar at 0%, with the not-yet-visible section's `Cell`s genuinely absent from the lazily-composed `UICollectionView`, not just off-screen). A bare `waitForExistence` on `layout-detail-duplicate-button`/`layout-detail-edit-keys-button`/a "My Layouts" row can time out even though the element renders fine once scrolled into range — this is NOT flakiness, it reproduces every time for layouts with enough preview rows (e.g. `ipa-full.json`, which has more rows than `en-US.json`). Fixed via a shared top-level helper `waitForRevealed(_:scrollingIn:timeout:maxSwipes:)` in `LibraryScreen.swift` — `LayoutDetailScreen.waitForContent`/`waitForUserLayoutContent`/`scrollTo`, `LibraryScreen.waitForRow(labelContains:)`/`waitForRow(labelContainsAll:)`, and `LayoutKeyEditorScreen.waitForRow(at:)`/`waitForEmptyRow`/`waitForAddRowButton`/`KeyRowEditorScreen.waitForKey(at:)` all use it. Hardened 2026-07-04 (issue #119 flake sweep): deceleration-free press-drag scroll steps, composition gate before the first scroll, two-strike progress-based exhaustion, then a bounded scan back toward the top — so `false` now GUARANTEES the whole list was scanned (absence probes like `waitForEmptyRow(...)==false` are no longer vacuous), and success after any scroll requires hittable + frame-stable (safe to tap immediately). Reuse this helper for any new list-based screen rather than a bare `waitForExistence`; see [[project_flake_hardening]] for the tap-side helpers built on it.

## App Group container is AVAILABLE in every lane that runs this suite (updated 2026-08-02, issue #210)

**This section previously said the opposite — do not trust older copies of it.** `LayoutStore.save`/`delete` (`IPAKeyboardKit/Store/LayoutStore.swift`) throw `StoreError.sharedContainerUnavailable` whenever `AppGroup.containerURL` is nil, which needs the App Group *entitlement* embedded in the running process — i.e. a code-signed build. Every lane that runs the UI suite now signs the app: locally automatic under the project's team, and in CI ad-hoc via `CODE_SIGN_STYLE=Manual CODE_SIGN_IDENTITY=-`, which needs **no certificate, provisioning profile, or Apple account** on a simulator destination (`-` resolves to "Sign to Run Locally"). Verified empirically 2026-08-02, not inferred: `simctl get_app_container <udid> net.yuryu.IPAKeyboard groups` prints a live `group.net.yuryu.IPAKeyboard` path for an ad-hoc-signed build and prints nothing for a `CODE_SIGNING_ALLOWED=NO` one. Practical fallout:
- **Fork-then-act flows are drivable end-to-end.** `KeyEditorUITests`' `duplicateBuiltInLayout(from:library:)` now asserts the fork persisted instead of returning a Bool, and the two flow tests (`test_editorFlow_editedKeyPersistsToDetailPreview`, `test_editorFlow_cancelWithChangesDiscardsDraft`) run for real. Their `XCTSkip` guards are **gone on purpose** — a skip reads as a pass, so the old guards would have hidden a signing regression. Don't reintroduce them.
- The two `…succeedsOrDegradesGracefully` tests were renamed to what they now assert: `test_duplicateBuiltIn_persistsForkedLayout` and `test_importValid_persistsImportedLayout`. Both still *poll* the "Something went wrong" alert alongside the success condition, but only so a container-less run names the shared-storage failure instead of timing out anonymously — the alert is a failure, not an accepted branch.
- Degraded-path (nil container) coverage lives in `IPAKeyboardTests/LayoutLibraryTests.swift`, which injects `LayoutStore(containerURL: nil)` deterministically. Don't re-cover it from the UI lane.
- **The suite is now stateful across tests.** Forked/imported user layouts persist across `app.launch()` calls *and across suites* in one test session. `LibraryScreen.resetLayoutsArgument` (`--uitest-reset-layouts`, issue #27) clears user layouts and per-layout prefs once per process before the first load; `AlternatesPopupUITests`, `ImportExportUITests`, `KeyEditorUITests`, and `LayoutListScratchpadUITests` pass it. Any new suite whose assertions depend on the library's contents must pass it too.
